...
// Enhanced Students Grades Table Component with 1-100 scale CPMK input
      ...
      ["tugas", "kuis", "uts", "uas"].forEach((assessmentType) => {
        ...
        relatedCPL.forEach((cpl) => {
          ...
          relatedCPMK.forEach((cpmk) => {
            ...
            // Calculate CPMK weight percentage based on assessment weights
            const getCpmkWeight = (cpmk: string, cpl: string) => {
              ...
              // Fallback: Auto-calculate equal distribution if no weights set
              const totalCpmkInCpl = relatedCPMK.length;
              return `${Math.round(100 / totalCpmkInCpl)}%`;
            };

            // Calculate Sub-CPMK weight percentage based on assessment weights
            const getSubCpmkWeight = (
              subCpmk: string,
              cpmk: string,
              cpl: string
            ) => {
              ...
              // Fallback: Auto-calculate equal distribution if no weights set
              const relatedSubCPMK = getRelatedSubCPMK(cpmk);
              const totalSubCpmkInCpmk = relatedSubCPMK.length;
              return totalSubCpmkInCpmk > 0
                ? `${Math.round(100 / totalSubCpmkInCpmk)}%`
                : "0%";
            };
            ...
            if (hasSubCPMKData && relatedSubCPMK.length > 0) {
              ...
              const subCpmkColumns: any[] = relatedSubCPMK.map((subCpmk) => ({
                title: (
                  <div className="text-center">
                    ...
                    <div className="text-xs bg-blue-200 px-1 rounded font-semibold">
                      {getSubCpmkWeight(subCpmk, cpmk, cpl)}
                    </div>
                  </div>
                ),
                ...
                render: (_: any, record: Student) => {
                  // Get stored Sub-CPMK score (direct 1-100) or calculate from weighted assessment
                  const storedScore =
                    (record[
                      `${cpl}_${cpmk}_${subCpmk}_${assessmentType}`
                    ] as number) || 0;
                  const calculatedScore = calculateCPMKScore(
                    record,
                    cpl,
                    cpmk,
                    assessmentType,
                  ...
                  if (record.key === "average") {
                    const avgField = isGradeInputMode
                      ? calculatedScore
                      : storedScore;
                    return (
                      <div
                  ...
                  if (!isGradeInputMode) {
                    // CPMK input mode: Direct 1-100 input
                    return (
                      <Input
                        type="number"
                        value={storedScore}
                    ...
                    // Grade input mode: Show calculated score
                    return (
                      <div
              ...
              cpmkColumns.push({
                ...
                render: (_: any, record: Student) => {
                  // Get stored CPMK score (direct 1-100) or calculate from weighted assessment
                  const storedScore =
                    (record[`${cpl}_${cpmk}_${assessmentType}`] as number) || 0;
                  const calculatedScore = calculateCPMKScore(
                    record,
                    cpl,
                    cpmk,
          ...
          <div className="flex items-center gap-4">
            ...
            <Text type="secondary">
              {isGradeInputMode
                ...
                : `(${
                    hasSubCPMKData ? "Sub-CPMK" : "CPMK"
                  } dapat diedit langsung dengan skala 1-100)`}
            </Text>
          </div>
        }
        type="info"
...
const GradingAssessmentTable: React.FC = () => {
  ...
  const recalculateAllStudents = (): void => {
    setStudents((prev) =>
      prev.map((student) => {
        ...
        // In CPMK input mode, calculate final score using direct CPMK scores
        if (!isGradeInputMode) {
          // Use direct CPMK/Sub-CPMK scores to calculate final score
          let totalScore = 0;
          ...
          relatedCPL.forEach((cpl) => {
            ...
            relatedCPMK.forEach((cpmk) => {
              ...
              if (hasSubCPMK && relatedSubCPMK.length > 0) {
                // Sub-CPMK calculation - only include if has weight
                relatedSubCPMK.forEach((subCpmk) => {
                  ["tugas", "kuis", "uts", "uas"].forEach((assessmentType) => {
                    const weight =
                      assessmentWeights[cpl]?.[cpmk]?.subcpmk?.[subCpmk]?.[
                    ...
                    // Only include in calculation if weight > 0
                    if (weight > 0) {
                      const score =
                        Number(
                          student[`${cpl}_${cpmk}_${subCpmk}_${assessmentType}`]
                        ) || 0;
                      totalScore += score;
                      totalComponents++;
                    }
                ...
                // CPMK calculation - only include if has weight
                ["tugas", "kuis", "uts", "uas"].forEach((assessmentType) => {
                  const weight =
                    assessmentWeights[cpl]?.[cpmk]?.[
                      assessmentType as keyof (typeof assessmentWeights)[string][string]
                  ...
                  if (weight > 0) {
                    const score =
                      Number(student[`${cpl}_${cpmk}_${assessmentType}`]) || 0;
                    totalScore += score;
                    totalComponents++;
                  }
                });
          ...
          // In grade input mode, only calculate if all basic assessment scores exist
          ...
          // Use weighted calculation based on assessment weights
  ...
  const initializeStudents = (count: number): void => {
    const newStudents: Student[] = Array.from({ length: count }, (_, i) => {
      ...
      // Initialize CPMK/Sub-CPMK score fields
      relatedCPL.forEach((cpl) => {
        const relatedCPMK = getRelatedCPMK(cpl);
        relatedCPMK.forEach((cpmk) => {
          const relatedSubCPMK = getRelatedSubCPMK(cpmk);
          ...
          if (hasSubCPMK && relatedSubCPMK.length > 0) {
            relatedSubCPMK.forEach((subCpmk) => {
              ["tugas", "kuis", "uts", "uas"].forEach((assessmentType) => {
                student[`${cpl}_${cpmk}_${subCpmk}_${assessmentType}`] = 0;
              });
            });
          } else {
            ["tugas", "kuis", "uts", "uas"].forEach((assessmentType) => {
              student[`${cpl}_${cpmk}_${assessmentType}`] = 0;
            });
          }
        });
      });
  ...
  const updateStudent = (
    ...
    setStudents((prev) =>
      prev.map((student) => {
        ...
        // Trigger recalculation if assessment scores or CPMK scores change
        if (
          ["tugas", "kuis", "uts", "uas"].includes(field as string) ||
          String(field).includes("_")
        ) {
          // Don't recalculate immediately for individual changes
          // recalculateAllStudents will be called separately
        }
    ...
    setTimeout(() => recalculateAllStudents(), 10);
  };
        ...
        <Card>
          <div className="text-center bg-green-100 p-4 rounded-t">
            <Title level={2} className="m-0">
              RUBRIK PENILAIAN OBE - Enhanced CPMK Input Mode
            </Title>
                ...
                {!isGradeInputMode ? (
                  ...
                  >
                    <Alert
                      ...
                      description={`Grafik radar akan menampilkan capaian setiap ${
                        ...
                        hasSubCPMK ? "Sub-CPMK" : "CPMK"
                      }. Data menggunakan skala 1-100 untuk setiap ${
                        hasSubCPMK ? "Sub-CPMK" : "CPMK"
                      }.`}
                      type="info"
                      icon={<InfoCircleOutlined />}
                    ...
                    <div className="text-center py-12 text-gray-500">
                      ...
                      <div>
                        ...
                        ditampilkan di sini berdasarkan data assessment dengan
                        skala 1-100
                      </div>
                    </div>
                  </Card>
                  ...
                  <Alert
                    ...
                    description={`Grafik radar hanya tersedia dalam mode Input ${
                      ...
                      hasSubCPMK ? "Sub-CPMK" : "CPMK"
                    } untuk melihat grafik radar capaian dengan skala 1-100.`}
                    type="info"
                    icon={<InfoCircleOutlined />}
                    showIcon
                  />
